...
div
	...
	aside.sidebar(:class='{show: showSidebar}')
		...
		.sidebar__credit.
			<a href='https://github.com/baku89/pentool' target='_blank'>GitHub Repo</a>
...
export default {
  ...
  methods: {
    recompileActiveTool() {
      ...
      this.$set(
        this.activeTool,
        "parameters",
        JSON.parse(this.editingParameters)
      );
    ...
    deleteTool() {
      ...
      this.activeToolIndex = Math.min(
        this.activeToolIndex,
        this.tools.length - 1
      );
    },
  ...
  data() {
    const tools =
      JSON.parse(localStorage.getItem("tools")) || ToolPresets.presets;

    return {
      tools,
      parameters: {},
      parametersCache:
        JSON.parse(localStorage.getItem("parametersCache")) || {},
